...
const commands = {
    "L": (position) => new Point(position.x - 1, position.y),
    "U": (position) => new Point(position.x, position.y + 1),
    "R": (position) => new Point(position.x + 1, position.y),
    "D": (position) => new Point(position.x, position.y - 1),
};
...
const addPoints = (callback) => move => {
    ...
    while (reps--) {
        let point = commands[move[0]](map.slice(-1)[0]);
        callback(point);
        map.push(point);
    }
};
...
let manhatanDistances = cross.map(p => p.calculateManhatanDistance());
